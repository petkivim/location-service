...
public class ExporterValidationFilter implements Filter {
    ...
    private final static Logger logger = Logger.getLogger(ExporterValidationFilter.class.getName());
    ...
    private boolean validate(ServletRequest req) {
        ...
        if (req.getParameter("owner") == null) {
            ...
            if (owner == null) {
                logger.warn("Bad request! Reason : owner \"" + ownerCode + "\" doesn't exist!");
                // "owner" is required -> invalid request
                return false;
            } else {
