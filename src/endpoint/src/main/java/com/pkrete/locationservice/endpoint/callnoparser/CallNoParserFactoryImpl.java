...
public class CallNoParserFactoryImpl implements CallNoParserFactory {
    ...
    private final static Logger logger = Logger.getLogger(CallNoParserFactoryImpl.class.getName());
    ...
    public CallNoParser createParser(Generator generator, String owner, LocatingStrategy strategy) {
        ...
        if (strategy == null) {
            logger.warn(new StringBuilder("Strategy parameter is null -> use \"SIMPLE\" call number parser. Owner : \"").append(owner).append("\"."));
        ...
        if (parser == null) {
            ...
            logger.warn("Parser matching the given \"" + strategy + "\" strategy is not available in the parsers list. Use \"SIMPLE\" strategy.");
            strategy = LocatingStrategy.SIMPLE;
        }
        // Set generator
        parser.setGenerator(generator);
        if (logger.isDebugEnabled()) {
            logger.debug(new StringBuilder("Use \"").append(strategy).append("\" call number parser. Owner : \"").append(owner).append("\"."));
        }
        // Return the parser
        return parser;
    }
