 ...
 * and an operation that's done, if the rule apply. Before doing the modification
 * it's possible to check if the given call number and modification rule are
 * matching.
 *
 * Both the rule and the operation can contain regular expressions. It is
 * possible to use the value of a group within the condition, and
 * it is also possible to use a group value in the operation pattern.
 * Instead of a backslash, the form of the reference is $n where n is a
 * group number starting from 0.
 *
...
public class CallnoModifier implements Modifier {
    ...
    /**
     * Compares the given call number and call number modification rule
     * and tells if the given call number matches the rule.
    ...
    /**
     * Modfies the given call number according to the given modification rule,
     * and returns the modified call number.
     * @param callno call number to be modifed
     * @param modification modification rule to be checked
