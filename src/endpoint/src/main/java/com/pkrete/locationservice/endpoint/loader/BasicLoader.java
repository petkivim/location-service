...
/**
 * The <code>OutputLoader</code> class implements the {@link Loader Loader}
 * interface.
 *
 * This class implements the methods defined in the OutputLoader interface. This
 * class offers the functionality for loading the template files that are used
 * for generating the html page that is returned to user. TemplateParser
 * interface is responsible of the actual parsing and HTML generation.
 *
 * @author Petteri Kivimäki
 */
public class BasicLoader implements Loader {

    private final static Logger logger = LoggerFactory.getLogger(BasicLoader.class.getName());
    private String encoding;
    private TemplateParser parser;

    /**
     * Constructs and initializes an OutputLoader object.
     */
    public BasicLoader() {
        this.encoding = "UTF-8";
    }
    ...
    public String loadTemplateNotFound(String lang, String callno, String owner) {
        ...
        if (!file.exists()) {
            logger.error("Template doesn't exist : \"{}\"", builder.toString());
            return "<html>\n<head>\n</head>\n<body>\n</body>\n</html>\n";
        }
        logger.debug("Template found : \"{}\"", builder.toString());
    ...
    public String loadTemplate(String lang, Shelf shelf, String callno) {
        ...
        if (!new File(fileName).exists()) {
            logger.error("Template doesn't exist : \"{}\"", fileName);
            return "<html>\n<head>\n</head>\n<body>\n</body>\n</html>\n";
        }
    ...
    private String getTemplateName(Shelf shelf, String lang) {
        ...
        logger.debug("Use default template : \"{}\"", builder.toString());
    ...
    public String getTemplateOtherPath(String template, String lang, String owner) {
        ...
        File file = new File(builder.toString());
        if (!file.exists()) {
            logger.warn("Template not found : \"{}\"", builder.toString());
            return null;
        }
        logger.debug("Template found : \"{}\"", builder.toString());
        return file.getAbsolutePath();
