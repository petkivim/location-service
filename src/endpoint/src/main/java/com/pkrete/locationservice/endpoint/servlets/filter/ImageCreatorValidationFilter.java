...
/**
 * This filter is run every time when ImageCreator servlet is accessed - all the
 * requests to the ImageCreator are passing through this filter. This filter
 * checks that the request contains all the required parameters and sets a
 * boolean attribute indicating the validity of the request. The request object
 * is wrapped inside ExporterRequest object that takes care of returning default
...
public class ImageCreatorValidationFilter implements Filter {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ImageCreatorValidationFilter.class.getName());
