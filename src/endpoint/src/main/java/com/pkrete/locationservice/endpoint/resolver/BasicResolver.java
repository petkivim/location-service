 ...
 * necessary tasks between other classes. LocationResolver can be seen as a 
 * link between the {@link LocationService LocationService} servlet and other 
...
public class BasicResolver extends Resolver {
    ...
    private final static Logger logger = Logger.getLogger(BasicResolver.class.getName());

    /**
     * Contructs and initializes a new BasicResolver object.
     */
    ...
    /**
     * Runs through preprocessing redirects and modifies the given call
     * number accordingly. If there are multiple modifications mathing the
     * call number, only first one is processed and all the others are 
     * skipped. If there are no redirects matching the call number,
     * the orignal call number is returned unchanged.
        ...
        for (CallnoModification mod : redirects) {
            if (modifier.canBeModified(callno, mod)) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(new StringBuilder("Found reprocessing redirect match!").append(" Redirect: \"").append(orgCallno).append("\" -> \"").append(callno).append("\"."));
                }
                return callno;
            }
        }
    ...
    /**
     * Runs through locations that have Match beginning checkbox checked and
    ...
    /**
     * Runs through locations that have Match beginning checkbox checked and
     * checks if the given call number matches to one of them, and if the
     * given collection code matches too. If a match is found, the matching 
     ...
     * matches to one of them. If a match is found, the matching Shelf object 
     * is returned.
     ...
     * @param owner owner code of the library
     * @param localService service object for data connections
     * @return mathichg Shelf object or null
     */
     ...
     * the given call number and collection code combination matches to one
     * of them. If a match is found, the matching LibraryCollection object is 
     * returned. If no match is found, null is returned.
     * @param collection collection code to be searched
     * @param callno call number to be searched
     * @param owner owner code of the library
     * @param localService service object for data connections
     * @return mathichg LibraryCollection object or null
     */
