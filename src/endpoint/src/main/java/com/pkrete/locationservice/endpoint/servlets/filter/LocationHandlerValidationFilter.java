...
/**
 * This filter is run every time when LocationHandler servlet is accessed - all
 * the requests to the LocationHandler are passing through this filter. This
 * filter checks that the request contains all the required parameters and sets
 * a boolean attribute indicating the validity of the request. The request
 * object is wrapped inside ExporterRequest object that takes care of returning
 * default values to optional parameters, if they're not present in the request.
 *
 * @author Petteri Kivimäki
 */
public class LocationHandlerValidationFilter implements Filter {

    private static final Logger logger = LoggerFactory.getLogger(LocationHandlerValidationFilter.class.getName());
