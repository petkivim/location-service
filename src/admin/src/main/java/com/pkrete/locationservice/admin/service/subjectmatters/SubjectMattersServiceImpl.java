...
public class SubjectMattersServiceImpl implements SubjectMattersService {
    ...
    private final static Logger logger = Logger.getLogger(SubjectMattersServiceImpl.class.getName());
    ...
    /**
     * Returns a list of all the subject matters in the database that are related
     ...
     * @param subjectMatter the subject matter to be addeed
    ...
    public boolean create(SubjectMatter subjectMatter) {
        ...
        if (dao.create(subjectMatter)) {
            if (logger.isInfoEnabled()) {
                logger.info(new StringBuilder("Subject matter created : ").append(this.jsonizer.jsonize(subjectMatter, true)));
            }
            this.addToIndex(subjectMatter);
            return true;
        }
        logger.warn(new StringBuilder("Failed to create subject matter : ").append(this.jsonizer.jsonize(subjectMatter, true)));
        return false;
    }
    ...
    public boolean update(SubjectMatter subjectMatter) {
        ...
        if (dao.update(subjectMatter)) {
            if (logger.isInfoEnabled()) {
                logger.info(new StringBuilder("Subject matter updated : ").append(json));
            }
            this.updateToIndex(subjectMatter);
            return true;
        }
        logger.warn(new StringBuilder("Failed to update subject matter : ").append(json));
        return false;
    }
    ...
    public boolean delete(SubjectMatter subjectMatter) {
        ...
        if (subjectMatter.getLocations() != null && !subjectMatter.getLocations().isEmpty()) {
            if (logger.isInfoEnabled()) {
                logger.info(new StringBuilder("Failed to delete a subject matter, because it has locations attached to it. Id : ").append(subjectMatter.getId()));
            }
            return false;
        }
        String json = this.jsonizer.jsonize(subjectMatter, true);
        if (dao.delete(subjectMatter)) {
            if (logger.isInfoEnabled()) {
                logger.info(new StringBuilder("Subject matter deleted : ").append(json));
            }
            this.deleteFromIndex(subjectMatter);
            return true;
        }
        logger.warn(new StringBuilder("Failed to delete subject matter : ").append(json));
        return false;
    }
    ...
    /**
     * Can be used for updating SubjectMatters to external index. Not implemented.
    ...
    /**
     * Can be used for deleting SubjectMatters from external index. Not implemented.
