 ...
 * before processing the given request. If the user isn't stored in the session,
 * the User object is fetched from the database and it's added to the session.
 * If fetching the user from the database fails, the handler execution chain is
 * stopped and the user is forwarded to the login page.
 *
 * This interceptor also validates that the current User and remote user are the
 * same. If the users don't match, the handler execution chain is stopped and
...
public class UserSessionInterceptor extends HandlerInterceptorAdapter {
    ...
    private final static Logger logger = LoggerFactory.getLogger(UserSessionInterceptor.class.getName());
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if (user == null && remoteUser != null) {
            ...
            if (user == null) {
                logger.warn("Unable to find User object matching the remote user \"{}\". Stop handler execution chain.", remoteUser);
            ...
            if (!user.getUsername().equals(remoteUser)) {
                logger.warn("Current user \"" + user.getUsername() + "\" and remote user \"{}\" don't match. Stop handler execution chain.", remoteUser);
                // Logout the current user by forwarding to logout controller
                request.getRequestDispatcher("/logout.htm").forward(request, response);
