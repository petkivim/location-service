...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.w3.org/MarkUp/SCHEMA/xhtml11.xsd" xml:lang="fi">
    ...
    <body>
        ...
        <div id="page_title"><fmt:message key="menu.users" /></div>
        <div id="main">
            <form action="userowner.htm"  accept-charset="UTF-8" method="POST">
                <table id="user_owner_table">
                    ...
                    <tr>
                        <td class="table_sub_title"><fmt:message key="label.owners.title" /></td>
                    </tr>
                    <tr>
                        <td class="user_owner_table_col1"><fmt:message key="label.owners.dropdown" /></td>
                        <td class="user_owner_table_col2" colspan="2">
                            <select id="select_owner" name="select_owner">
                                <c:forEach items="${model.owners}" var="owner">
                                    <c:choose>
                                        <c:when test="${param.select_owner == owner.id}">
                        ...
                        <td class="user_owner_table_col3">
                            <input class="button" type="submit" name="btn_edit_owner" value="<fmt:message key="btn.edit" />" />
                            <input class="button" type="submit" name="btn_add_owner" value="<fmt:message key="btn.add" />" />
                            <input class="button" type="button" name="delete_owner" value="<fmt:message key="btn.delete" />" />
                            <input type="submit" name="btn_delete_owner" class="invisible" />
                        </td>
                    </tr>
