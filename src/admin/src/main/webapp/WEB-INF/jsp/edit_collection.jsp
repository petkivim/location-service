...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.w3.org/MarkUp/SCHEMA/xhtml11.xsd" xml:lang="fi">
    <head>
        ...
        <title><fmt:message key="title.service" /> - <fmt:message key="title.locations.edit.collection" /></title>
    </head>
    <body>
        <jsp:include page="main_menu.jsp" />
        <div id="page_title"><fmt:message key="title.locations.edit.collection" /></div>
        <div id="main">
            <div class="history">
                <fmt:message key="title.created" /> <fmt:formatDate pattern="dd.MM.yyyy HH:mm" value="${collection.created}" /> (${collection.creator}) |
                <fmt:message key="title.updated" /> <fmt:formatDate pattern="dd.MM.yyyy HH:mm" value="${collection.updated}" /> (${collection.updater})
            </div>
            <form:form acceptCharset="UTF-8" method="POST" commandName="collection">
                <table id="add_location_table">
                    <tr>
                        <td class="add_location_table_col1"><fmt:message key="label.locations.addedit.library.dropdown" /></td>
                        <td class="add_location_table_col2" colspan="2">
                            <form:select path="library" id="library">
                                <c:forEach items="${libraries}" var="lib">
                                    <c:choose>
                                        <c:when test="${collection.library.locationId == lib.locationId}">
                                            <form:option value="${lib.locationId}" label="${lib.name}" selected="true" />
                                        </c:when>
                                        <c:otherwise>
                                            <form:option value="${lib.locationId}" label="${lib.name}" />
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                            </form:select>
                    ...
                    <tr>
                        <td class="add_location_table_col1"><fmt:message key="label.locations.addedit.substring" /></td>
                        <td class="add_location_table_col2"><form:checkbox path="isSubstring"/></td>
                        <td class="add_location_table_col3"></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="add_location_table_col1"><fmt:message key="label.locations.addedit.collection_code" /></td>
                        <td class="add_location_table_col2"><form:input path="collectionCode" /></td>
                        <td class="add_location_table_col3"><form:errors path="collectionCode" cssClass="error" /></td>
                    </tr>
                    <c:forEach items="${collection.descriptions}" var="i" varStatus="itemsRow">
                        <tr>
                    ...
                    <c:forEach items="${collection.notes}" var="i" varStatus="itemsRow">
                        <tr>
                    ...
                    <tr>
                        ...
                        <td class="add_location_table_col2">
                            ...
                            <input class="button" type="button" name="back" id="btn_back_collection" value="<fmt:message key="btn.back" />" />
                        </td>
                        <td class="add_location_table_col3"></td>
                    </tr>
                </table>
