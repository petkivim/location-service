...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.w3.org/MarkUp/SCHEMA/xhtml11.xsd" xml:lang="fi">
    <head>
        ...
        <title><fmt:message key="title.service" /> - <fmt:message key="title.locations.edit.shelf" /></title>
    </head>
    <body>
        <jsp:include page="main_menu.jsp" />
        <div id="page_title"><fmt:message key="title.locations.edit.shelf" /></div>
        <div id="main">
            <div class="history">
                <fmt:message key="title.created" /> <fmt:formatDate pattern="dd.MM.yyyy HH:mm" value="${shelf.created}" /> (${shelf.creator}) |
                <fmt:message key="title.updated" /> <fmt:formatDate pattern="dd.MM.yyyy HH:mm" value="${shelf.updated}" /> (${shelf.updater})
            </div>
            <form:form acceptCharset="UTF-8" method="POST" commandName="shelf">
                <table id="add_location_table">
                    <tr>
                        <td class="add_location_table_col1"><fmt:message key="label.locations.addedit.collection.dropdown" /></td>
                        <td class="add_location_table_col2" colspan="2">
                            <form:select path="collection" id="collection">
                                <c:forEach items="${collections}" var="col">
                                    <c:choose>
                                        <c:when test="${shelf.collection.locationId == col.locationId}">
                                            <form:option value="${col.locationId}" label="${col.name}" selected="true" />
                                        </c:when>
                                        <c:otherwise>
                                            <form:option value="${col.locationId}" label="${col.name}" />
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                            </form:select>
                ...
                <input type="hidden" id="select_shelf" name="select_shelf" value="${param.select_shelf}" />
            </form:form>
        </div>
        <div class="invisible">
